...
public:
	...
	struct Shape
	{
		...
		public:
			...
			/// @ingroup Box
			inline Polygon(float x, float y, Vector2 center, Rotation rotation) : m_shape(b2MakeOffsetBox(x/2, y/2, (b2Vec2)center, rotation.getRadians())) {}
			...
			/// @exception asserts that the hull is valid
			/// @ingroup Polygon
			inline Polygon(const Hull& hull, float radius, Vector2 center, Rotation rotation)
			{
				assert(hull.isValid() && "Hull must be valid to create Polygon!"); 
				m_shape = b2MakeOffsetPolygon(&hull.m_hull, radius, b2Transform{(b2Vec2)center, (b2Rot)rotation});
			}
			...
			{ this->m_shape = b2MakeOffsetBox(x/2, y/2, (b2Vec2)center, rotation.getRadians()); }
			/// @brief fills this polygon with points from the given hull
			/// @param radius how the polygon will be rounded (0 means no rounding). Radius is added around the entire shape and allows for a curve at edges
			/// @exception asserts that the hull is valid
			/// @ingroup Polygon
			...
			/// @exception asserts that the hull is valid
			/// @ingroup Polygon
			inline void makeOffsetPolygon(const Hull& hull, float radius, Vector2 center, Rotation rotation) 
			{ 
				assert(hull.isValid() && "Hull must be valid to create Polygon!"); 
				m_shape = b2MakeOffsetPolygon(&hull.m_hull, radius, b2Transform{(b2Vec2)center, (b2Rot)rotation});
			}
			...
			{ 
				this->m_shape = b2MakeOffsetBox(sideLengths/2, sideLengths/2, (b2Vec2)center, rotation.getRadians());
			}
	...
	b2CastOutput rayCast(Vector2 origin, Vector2 translation); // TODO implement a wrapper for b2CastOutput
	...
	// /// @returns stored smooth line segment data 
	// /// @note These come from chain shapes
	// /// @warning Asserts the type is correct
	// b2SmoothSegment getSmoothSegment() const;
	...
	// /// @brief this will only work if the current type is a b2_smoothSegmentShape
	// /// @note multiple b2_smoothSegmentShape can be connected to one chain
	// /// @warning asserts that the type is correct
	// /// @returns a pointer to the chain which contains this fixture 
	// Fixture::Shape::Chain getChain() const;
